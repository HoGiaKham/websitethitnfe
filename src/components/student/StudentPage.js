// src/components/student/StudentPage.js
  ...
  return (
    <div className="student-page">
      <div className="page-header">
        ...
        <p>Xin chào, <strong>{studentUsername}</strong>! Bạn đang tham gia <strong>{classes.length}</strong> lớp.</p>
      </div>
          ...
          {classes.map((cls) => (
            ...
            >
              ...
              <div className="class-info">
                <p>
                  <strong>Giảng viên:</strong> {cls.teacher?.name || cls.teacher?.username || "Chưa có"}
                </p>
                <p>
                  <strong>Môn học:</strong> {cls.subject?.name || "N/A"}
                </p>
                <p>
                  <strong>Số lượng sinh viên:</strong> {cls.students?.length || 0}/{cls.maxStudents || "∞"}
                </p>
              </div>
              {/* <button
                className="view-btn"
  ...
  const fetchExams = async () => {
    ...
    try {
      ...
      if (err.response?.status === 400) {
        console.warn("400: Thiếu classId/subjectId hoặc không hợp lệ");
      } else {
                ...
                <div key={exam._id} className="exam-item">
                  <div className="exam-info">
                    ...
                    <div className="exam-meta">
                      ...
                      {exam.closeTime && <span>Đóng: {formatDate(exam.closeTime)}</span>}
                    </div>
                  ...
                  <button
                    ...
                    disabled={!open}
                    className={`start-exam-btn ${open ? "active" : "disabled"}`}
                  >
